...
namespace Clipper2Lib {
  ...
  inline Location GetAdjacentLocation(const Location &loc, const bool &isClockwise)
  {
    int delta = (isClockwise) ? 1 : 3;
    return static_cast<Location>((static_cast<int>(loc) + delta) % 4);
  }

  inline bool HeadingClockwise(const Location &prev, const Location &curr)
  {
    return (static_cast<int>(prev) + 1) % 4 == static_cast<int>(curr);
  }

  inline bool AreOpposites(const Location &prev, const Location &curr)
  {
    return abs(static_cast<int>(prev) - static_cast<int>(curr)) == 2;
  }

  inline bool IsClockwise(const Location& prev, const Location &curr,
  ...
  inline bool IsHeadingClockwise(const Point64& pt1, const Point64& pt2, const int& edgeIdx)
  {
    switch (edgeIdx)
    {
  ...
  inline void SetNewOwner(OutPt2* op, const size_t &new_idx)
  {
  ...
  OutPt2* RectClip64::Add(const Point64& pt, const bool &start_new)
  {
  ...
  void RectClip64::AddCorner(const Location &prev, const Location &curr)
  {
  ...
  void RectClip64::AddCorner(Location& loc, const bool &isClockwise)
  {
    if (isClockwise)
    {
  ...
  void RectClip64::TidyEdges(const size_t& idx, OutPt2List& cw, OutPt2List& ccw)
  {
    if (ccw.empty()) return;
