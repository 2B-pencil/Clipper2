...
inline bool AlmostZero(const double &value, const double &epsilon = 0.001)
{
...
static inline Point64 GetPerpendic(const Point64& pt, const PointD& norm, const double &delta)
{
#ifdef USINGZ
...
inline PointD GetPerpendicD(const Point64& pt, const PointD& norm, const double &delta)
{
#ifdef USINGZ
...
ClipperOffset::Group::Group(const Paths64& _paths, const JoinType &_join_type, const EndType &_end_type):
...
void ClipperOffset::AddPath(const Path64& path, const JoinType &jt_, const EndType &et_)
{
	Paths64 paths;
...
void ClipperOffset::AddPaths(const Paths64 &paths, const JoinType &jt_, const EndType &et_)
{
	if (paths.size() == 0) return;
	groups_.emplace_back(Group(paths, jt_, et_));
}
...
void ClipperOffset::DoBevel(const Path64& path, const size_t& j, const size_t &k)
{
	PointD pt1, pt2;
	if (j == k)
	{
	...
	path_out.emplace_back(Point64(pt1));
	path_out.emplace_back(Point64(pt2));
}

void ClipperOffset::DoSquare(const Path64& path, const size_t &j, const size_t &k)
{
	PointD vec;
	if (j == k)
	...
	if (j == k)
	{
		...
		path_out.emplace_back(Point64(ReflectPoint(pt, ptQ)));
		path_out.emplace_back(Point64(pt));
	}
	else
	{
...
void ClipperOffset::DoMiter(const Path64& path, const size_t &j, const size_t &k, const double &cos_a)
{
	double q = group_delta_ / (cos_a + 1);
#ifdef USINGZ
	path_out.emplace_back(Point64(
...
void ClipperOffset::DoRound(const Path64& path, const size_t &j, const size_t &k, const double &angle)
{
	if (deltaCallback64_) {
...
#ifdef USINGZ
	path_out.emplace_back(Point64(pt.x + offsetVec.x, pt.y + offsetVec.y, pt.z));
#else
	path_out.emplace_back(Point64(pt.x + offsetVec.x, pt.y + offsetVec.y));
#endif
...
void ClipperOffset::OffsetPoint(Group& group, const Path64& path, const size_t &j, const size_t &k)
{
...
#ifdef USINGZ
		path_out.emplace_back(Point64(GetPerpendic(path[j], norms[k], group_delta_), path[j].z));
		path_out.emplace_back(path[j]); // (#405, #873, #916)
		path_out.emplace_back(Point64(GetPerpendic(path[j], norms[j], group_delta_), path[j].z));
#else
...
void ClipperOffset::ExecuteInternal(const double &delta)
{
	error_code_ = 0;
...
void ClipperOffset::Execute(const double &delta, Paths64& paths)
{
	paths.clear();
	solution = &paths;
...
void ClipperOffset::Execute(const double &delta, PolyTree64& polytree)
{
	polytree.Clear();
